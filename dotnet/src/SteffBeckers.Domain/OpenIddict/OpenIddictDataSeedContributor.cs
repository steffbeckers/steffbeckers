...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("SteffBeckers") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
            {
                Name = "SteffBeckers",
                DisplayName = "SteffBeckers API",
                Resources = { "SteffBeckers" }
            });
        }
    }
    ...
    {
        List<string> commonScopes = new List<string> {
        ...
        // CRM client
        string? appClientId = configurationSection["SteffBeckers_CRM:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
            string appRootUrl = configurationSection["SteffBeckers_CRM:RootUrl"]!.TrimEnd('/');

            await CreateApplicationAsync(
                name: appClientId,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Steff Beckers CRM",
                secret: null,
        ...
        // Swagger Client
        string? swaggerClientId = configurationSection["SteffBeckers_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            string? swaggerRootUrl = configurationSection["SteffBeckers_Swagger:RootUrl"]?.TrimEnd('/');

            await CreateApplicationAsync(
                name: swaggerClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Steff Beckers Swagger",
                secret: null,
                grantTypes: new List<string> { OpenIddictConstants.GrantTypes.AuthorizationCode, },
                scopes: commonScopes,
                redirectUri: $"{swaggerRootUrl}/oauth2-redirect.html",
